#!/bin/bash
set -e
cd /Users/carminecolarusso/Desktop/Claude/U1Dynamics-WEBPAGE

PAGE="src/app/page.tsx"
CSS="src/app/globals.css"

[ ! -f "$PAGE" ] && echo "ERROR: $PAGE missing" && exit 1
...
/* === Trust Strip ============================================ */
.trust-strip { border-top: 1px solid var(--steel-700, #2a2d31); border-bottom: 1px solid var(--steel-700, #2a2d31); background: var(--steel-950, #0d0e10); padding: 3rem 0; }
.trust-strip + .trust-strip { border-top: 0; }
.trust-inner { max-width: 1400px; margin: 0 auto; padding: 0 2rem; }
.trust-label { font-family: 'JetBrains Mono', ui-monospace, monospace; font-size: 11px; font-weight: 500; letter-spacing: 0.22em; text-transform: uppercase; color: var(--steel-400, #8a8f96); margin-bottom: 1.75rem; display: flex; align-items: center; gap: 0.75rem; }
.trust-label::before { content: ""; width: 24px; height: 1px; background: var(--ignition, #d84a1d); }
.trust-row { display: grid; grid-template-columns: repeat(6, 1fr); gap: 2rem; align-items: center; }
.trust-item { display: flex; flex-direction: column; align-items: center; justify-content: center; gap: 0.75rem; padding: 1rem 0.5rem; min-height: 88px; }
.trust-item img { max-width: 100%; max-height: 52px; width: auto; height: auto; object-fit: contain; filter: grayscale(100%) brightness(0) invert(0.72); opacity: 0.75; transition: opacity 200ms ease, filter 200ms ease; }
.trust-item:hover img { opacity: 1; filter: grayscale(0%) brightness(1) invert(0); }
.trust-item.retail img { max-height: 36px; }
.trust-caption { font-family: 'JetBrains Mono', ui-monospace, monospace; font-size: 9.5px; font-weight: 500; letter-spacing: 0.15em; text-transform: uppercase; color: var(--steel-500, #6b7177); text-align: center; line-height: 1.3; }
@media (max-width: 900px) { .trust-row { grid-template-columns: repeat(3, 1fr); gap: 1.25rem; } .trust-item { min-height: 72px; } .trust-item img { max-height: 40px; } .trust-item.retail img { max-height: 28px; } .trust-strip { padding: 2rem 0; } }
...
anchors = [r'(\s*\{/\*\s*===\s*Manifesto)', r'(\s*<section[^>]*className="manifesto)', r'(\s*<section[^>]*id="manifesto)']
inserted = False
for pat in anchors:
    m = re.search(pat, src)
    if m:
...
if not inserted:
    ...
    if len(sections) >= 2:
        ...
        print(f"✓ Fallback: inserted before section #2")
        inserted = True

if not inserted:
    print("ERROR: no insertion point"); sys.exit(1)
